...
export default function RecentActivity() {
  const [events] = useRecoilState(getEvents);

  return (
    <Box mb={4} borderRadius="md" background="gray.900" pt={5} px={6}>
      <Heading size="lg" fontWeight="light">
        <Icon as={BsCalendarWeek} boxSize={5} mr={2} />
        Recent Activity
      </Heading>
      {events.length ? (
        events
          ...
          .map((event, ind) => {
            return (
              <Box
                borderBottom={
                  ind + 1 != events.length && "1px solid rgba(255,255,255,0.1)"
                }
                py={4}
                key={ind}
              >
                  ...
                  <Box ml="auto" transform="translateY(-2px)">
                    {tokenId == undefined && event.tokenId && (
                      ...
                      >
                        ...
                        />{" "}
                        Grant #{event.tokenId.toNumber()}
                      </Text>
                    )}
                    <Box d="inline-block" mr={4} fontSize="xs">
                      <Icon
                        as={BsSquare}
                        mr={0.5}
                      ...
                      <EtherscanLink
                        ...
                        path={`/tx/${event.transactionHash}#eventlog`}
                        isExternal
                      >
                    ...
                    <Text d="inline-block" fontSize="xs">
                      <TimeIcon transform="translateY(-1px)" mr={0.5} />{" "}
                ...
                {event.type == "Redemption" && (
                  <Text fontSize="lg" mt={2}>
                    {parseFloat(
                      ethers.utils.formatUnits(event.amount, 18)
                    ).toLocaleString()}{" "}
                    {event.tokenSymbol} redeemed by {event.redeemerAddress}.
                  </Text>
                )}
              </Box>
            );
