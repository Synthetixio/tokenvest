...
export default function TokenBalance() {
  ...
  useEffect(() => {
    (async function () {
      const provider = new ethers.providers.Web3Provider(window?.ethereum);
      let newStats = [];
      const groupedGrants = groupBy(
        grants.filter((grant) => !grant.cancelled),
        "tokenAddress"
      );
      for (let grantGroup of Object.values(groupedGrants)) {
        const erc20Contract = new ethers.Contract(
          grantGroup[0].tokenAddress,
          erc20Abi.abi,
          provider
        );
        ...
        newStats.push({
          ...
          redeemed: parseFloat(
            ethers.utils.formatUnits(
              grantGroup.reduce(
                (acc, g) => acc.add(g.amountRedeemed),
                ethers.BigNumber.from(0)
              ),
              decimals
            )
          ),
          redeemable: parseFloat(
            ethers.utils.formatUnits(
              grantGroup.reduce(
                (acc, g) => acc.add(g.amountAvailable),
                ethers.BigNumber.from(0)
              ),
              decimals
            )
  ...
  const loadData = async () => {
    setLoadingData(true);
    fetch(
      `https://api.ethplorer.io/getAddressInfo/${contractAddress}?apiKey=freekey`
    )
  ...
  return (
    <Box mb={8} borderRadius="md" background="gray.900" py={5} px={6}>
      <Heading size="lg" fontWeight="light">
        <Icon as={BsCashStack} boxSize={5} mr={2} />
        Token Balances
      </Heading>
      <Text fontSize="sm" mt="2" mb="4">
        Grant recipients can redeem their tokens from{" "}
        <EtherscanLink
          d="inline"
          ...
          path={`/address/${process.env.NEXT_PUBLIC_VESTER_CONTRACT_ADDRESS}`}
          isExternal
        >
        ...
        (loadingData ? (
          ...
          <Table size="sm" variant="simple" mb={3}>
            ...
            <Tbody>
              {tokenData.map((token) => {
                return (
                  <Tr key={token.tokenInfo.symbol + "token-balance"}>
          ...
          <Text mt={16} mb={14} textAlign="center" opacity={0.8}>
          ...
          <>
            ...
            <StatGroup mb="3">
              ...
              <Stat>
                ...
                <StatNumber>
                  {statsGroup.redeemable.toLocaleString()}
                </StatNumber>
              </Stat>
              <Stat>
      ...
      {chainId == 1 && (
        <Text fontSize="xs" textAlign="center" opacity={0.8}>
          ...
          <Link
            ...
            href={`https://ethplorer.io/address/${contractAddress}`}
            isExternal
          >
            ethplorer
          </Link>
